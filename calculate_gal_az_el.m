function [az_result, el_result] = calculate_gal_az_el(time, gal_id, site_pos_lla) 
    ...
    % for Galileo specific ephemeris
    eph = ephemeris.Galileo;
    for i = 1 : num_sat
        prn = gal_id(i);
        ...
        for sec = 0 : total_secs
            ...
            % Note: Galileo System Time (GST) is nearly identical to GPS time for this calculation
...
function [pos, clk_err] = sat_position(eph, t)
    ...
    GM             = 3.986004418e14;   % Earth's universal gravitational constant for Galileo (GTRF),
                                       % [m^3/s^2]
    ...
    % Assuming standard MATLAB rinexread field name. If using E5b, change to BGD_E5bE1.
    clk_err = eph.SVClockBias + eph.SVClockDrift * dt + eph.SVClockDriftRate * dt^2 - eph.BGDE5aE1;
    ...
    % Note: OMEGA0 and OMEGA_DOT use the same variable names for Galileo in rinexread
    ...
    clk_err = eph.SVClockBias + eph.SVClockDrift * dt + eph.SVClockDriftRate * dt^2 + dtr - eph.BGDE5aE1;
end
...
function [az, el, d] = calculate_az_el(site_lla, sat_ecef)
    ...
    a = 6378137.0; % WGS84 semi-major axis (Galileo uses GTRF, essentially identical for ENU conversion)
    f = 1/298.257223563;
    e2 = 2*f - f^2;
